...
void inverse_cholesky_stride_batched_gpu(cublasHandle_t handle, int n, Array As, Array aInvs, int batchSize) {
    ...
    TIMER_LOG(inverse_cholesky_stride_batched_gpu_mem_htod, batchSize, n)
    TIMER_LOG(inverse_cholesky_stride_batched_gpu_ker, batchSize, n)
    TIMER_LOG(inverse_cholesky_stride_batched_gpu_mem_dtoh, batchSize, n)
#endif // DETAILED_LOGGING
...
void inverse_cholesky_batched_gpu(cublasHandle_t handle, int n, Array As, Array aInvs, int batchSize) {
    ...
    TIMER_LOG(decompose_cholesky_batched_gpu_mem_htod, batchSize, n)
    TIMER_LOG(decompose_cholesky_batched_gpu_ker, batchSize, n)
    TIMER_LOG(decompose_cholesky_batched_gpu_mem_dtoh, batchSize, n)
#endif // DETAILED_LOGGING
...
__global__
void decompose_cholesky_mm_kernel_device(Array *a, int N) {
    ...
    if (threadIdx.x == 0) {
        ...
        for (int i = 0; i < N; i++) {
            for (int j = 0; j <= i; j++) {
                MM[MMIDX(i, j, N)] = a[blockIdx.x][MATIDX(i, j, N)];
                //printf("[%d][%d]\n", i, j);
            }
        }
    }
    ...
    for (int k = 0; k < N; k++) {
        ...
        if (threadIdx.x == 0) { // computing diagonal elements in the first thread
            ...
            MM[MMIDX(k, k, N)] = sqrt(MM[MMIDX(k, k, N)]);
            //printf("[%d][%d] = sqrt([%d][%d])\n", k, k, k, k);
        ...
        if (i < N) {
            for (int j = i; j < N; j++) {
                MM[MMIDX(j, i, N)] -= MM[MMIDX(i, k, N)] * MM[MMIDX(j, k, N)];
                //printf("[%d][%d] -= [%d][%d] * [%d][%d]\n", j, i, i, k, j, k);
            }
        }
    }
    ...
    if (threadIdx.x == 0) {
        ...
        for (int i = 0; i < N; i++) {
            for (int j = 0; j <= i; j++) {
                a[blockIdx.x][MATIDX(j, i, N)] = 0;
                a[blockIdx.x][MATIDX(i, j, N)] = MM[MMIDX(i, j, N)];
                //printf("[%d][%d]\n", i, j);
            }
        }
    }
...
__global__
void inverse_lower_mm_kernel_device(Array *a, Array *aInv, int N) {
    ...
    for (int j = 0; j < N; j++) {
        for (int i = j + 1; i < N; i++) {
            ...
            if (threadIdx.x == 0) {
                MM[MMIDX(i, j, N)] = 0 - MM[MMIDX(i, j, N)] / (MM[MMIDX(j, j, N)] * MM[MMIDX(i, i, N)]);
                //printf("[%d][%d] = - [%d][%d] / [%d][%d] / [%d][%d]\n", i, j, i, j, j, j, i, i);
            }
            __syncthreads();

            int k = threadIdx.x + j + 1;
            if (k < i) {
                //printf("%f\n", MM[MMIDX(i, j, N)]);
                atomicAdd((float*)&MM[MMIDX(i, j, N)], 0 - MM[MMIDX(i, k, N)] * MM[MMIDX(k, j, N)] / MM[MMIDX(i, i, N)]);
                //printf("[%d][%d] -= [%d][%d] * [%d][%d] / [%d][%d] %f\n", i, j, i, k, k, j, i, i, MM[MMIDX(i, j, N)]);
            }
        }
    }
    __syncthreads();
    ...
    if (threadIdx.x == 0) {
        // copy from shared to global
        for (int i = 0; i < N; i++) {
            MM[MMIDX(i, i, N)] = 1.0 / MM[MMIDX(i, i, N)];
            for (int j = 0; j <= i; j++) {
                aInv[blockIdx.x][MATIDX(j, i, N)] = 0;
                aInv[blockIdx.x][MATIDX(i, j, N)] = MM[MMIDX(i, j, N)];
            }
        }
    }
}
...
__global__
void multiply_lower_mm_kernel_device(Array *a, Array *aInv, int N) {
    ...
    if (threadIdx.x == 0) {
        ...
        for (int i = 0; i < N; i++) {
            for (int j = 0; j <= i; j++) {
                MM[MMIDX(i, j, N)] = aInv[blockIdx.x][MATIDX(i, j, N)];
            }
        }
    ...
    for (int j = 0; j < N; j++) {
        for (int i = j; i < N; i++) {
            ...
            if (threadIdx.x == 0) {
                MM[MMIDX(i, j, N)] = MM[MMIDX(i, j, N)] * MM[MMIDX(i, i, N)] ;
            }
            __syncthreads();

            int k = threadIdx.x + i + 1;
            if (k < N) {
                atomicAdd((float*)&MM[MMIDX(i, j, N)], MM[MMIDX(k, j, N)] * MM[MMIDX(k, i, N)]);
                //printf("[%d][%d] -= [%d][%d] * [%d][%d] / [%d][%d] %f\n", i, j, i, k, k, j, i, i, MM[MMIDX(i, j, N)]);
            }
        }
    }
    ...
    if (threadIdx.x == 0) {
        ...
        for (int i = 0; i < N; i++) {
            for (int j = 0; j <= i; j++) {
                aInv[blockIdx.x][MATIDX(i, j, N)] = MM[MMIDX(i, j, N)];
                aInv[blockIdx.x][MATIDX(j, i, N)] = MM[MMIDX(i, j, N)];
            }
        }
    }
}
...
void inverse_cholesky_mm_batched_device(cublasHandle_t handle, int N, Array *devAs, Array *devAInvs, int batchSize) {
    ...
    inverse_lower_mm_kernel_device<<< batchSize, N, MMDIM(N) * sizeof(DataType)>>>(devAs, devAInvs, N);
    multiply_lower_mm_kernel_device<<< batchSize, N, MMDIM(N) * sizeof(DataType)>>>(devAs, devAInvs, N);
}
...
void inverse_cholesky_mm_batched_gpu(cublasHandle_t handle, int n, Array As, Array aInvs, int batchSize) {
    ...
    TIMER_LOG(decompose_cholesky_mm_batched_gpu_mem_htod, batchSize, n)
    TIMER_LOG(decompose_cholesky_mm_batched_gpu_ker, batchSize, n)
    TIMER_LOG(decompose_cholesky_mm_batched_gpu_mem_dtoh, batchSize, n)
#endif // DETAILED_LOGGING
...
void inverse_cholesky_mm2_batched_device(cublasHandle_t handle, int N, Array *devAs, Array *devAInvs, int batchSize) {
    decompose_cholesky_mm_kernel_device<<< batchSize, N, MMDIM(N) * sizeof(DataType)>>>(devAs, N);
    lu_inverse_batched_device(handle, N, devAs, devAInvs, batchSize);
}
...
#ifdef DETAILED_LOGGING
    TIMER_INIT(cholesky_mm2_batched_gpu_mem_htod)
    TIMER_INIT(cholesky_mm2_batched_gpu_ker)
    TIMER_INIT(cholesky_mm2_batched_gpu_mem_dtoh)
#endif // DETAILED_LOGGING
...
#ifdef DETAILED_LOGGING
    TIMER_START(cholesky_mm2_batched_gpu_mem_htod)
#endif // DETAILED_LOGGING
...
#ifdef DETAILED_LOGGING
    TIMER_STOP(cholesky_mm2_batched_gpu_mem_htod)
    TIMER_START(cholesky_mm2_batched_gpu_ker)
#endif // DETAILED_LOGGING

    inverse_cholesky_mm2_batched_device(handle, n, devAs, devAInvs, batchSize);

#ifdef DETAILED_LOGGING
    gpuErrchk( cudaPeekAtLastError() );
    gpuErrchk( cudaDeviceSynchronize() );

    TIMER_STOP(cholesky_mm2_batched_gpu_ker)
    TIMER_START(cholesky_mm2_batched_gpu_mem_dtoh)
#endif // DETAILED_LOGGING
...
#ifdef DETAILED_LOGGING
    TIMER_STOP(cholesky_mm2_batched_gpu_mem_dtoh)

    TIMER_LOG(cholesky_mm2_batched_gpu_mem_htod, batchSize, n)
    TIMER_LOG(cholesky_mm2_batched_gpu_ker, batchSize, n)
    TIMER_LOG(cholesky_mm2_batched_gpu_mem_dtoh, batchSize, n)
#endif // DETAILED_LOGGING
...
int main2(int argc, char const *argv[]) {
    cublasHandle_t handle;
    const char *directory = "tests/inverse_100_64x64";
    ...
    //readMatricesFile(filePath, &numMatrices, &m, &n, &a);
    //readMatricesFile(filePath, &numMatrices, &m, &n, &ainv);
    //inverse_cholesky_mm2_batched_gpu(handle, n, a, ainv, 1);
